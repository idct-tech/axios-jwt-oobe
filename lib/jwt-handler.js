...
class JwtHandler {
    ...
    setLoginAction(loginActionInfo) {
        const type = typeof loginActionInfo;
        if (type === "function") {
    ...
    login(username, password) {
        return new Promise((resolve, reject) => {
            const type = typeof this.loginActionInfo;
            if (type === "function") {
     ...
     */
    getToken() {
        if (typeof(window) !== "undefined" && typeof(localStorage) !== "undefined" && this.useLocalStorage) {
    ...
    /**
     * Adds authorization headers to the request config object.
     *
     * @param {object} config
     */
    updateConfig(config)
    ...
    /**
     * Handles the request interceptor:  adds authorization headers if required.
     *
     * @param {object} requestConfig
     */
    requestInterceptorAction(requestConfig) {
        if (!this.getToken()) {
            return requestConfig; //returns same instance as required by interceptors
        }

        this.updateConfig(requestConfig);
        return requestConfig; //returns same instance as required by interceptors
    }
        ...
        if (401 === error.response.status && error.config.secondAttempt !== true && this.getRefreshToken() && error.config.url !== this.refreshTokenRetrievalUrl) {
            return new Promise(async (resolve, reject) => {
                await this.axiosJwt.post(this.refreshTokenRetrievalUrl, {
                    ...
                    this.updateConfig(config); //adds the required headers
